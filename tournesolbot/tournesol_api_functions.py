...
def get_good_video(from_top,days_ago,language='en'):
    ...
    response = requests.get(f"https://api.tournesol.app/video/?language={language}&date_gte={date_gte}&limit={from_top}").json()
    ...
    n_contributor = 2
    print(df[['video_id','name','uploader','tournesol_score','rating_n_contributors','reliability']])
